...
- **强制简洁**：除非明确要求详细说明，否则回复必须简短直接，避免过度阐述
...
- **注释**: 最少化
  - ❌ 不要: 明显的代码、签名中已有的类型信息、分段分隔符
  - ✅ 要: 复杂算法、不明显的业务逻辑、公共 API 的 JSDoc
  - TODO: `// TODO: 描述 - 负责人`
...
| 配置文件 | `yaml` / `json` | ` ```yaml` |
| 脚本代码 | `javascript` / `ts` / `typescript` | ` ```ts` |
| Vue 组件 | `vue` | ` ```vue` |
...
- 标点符号：使用中文标点(，。：、；？！)
- 引号：使用「」或 “”
- 中英文混排：中英文之间、中文与数字之间加空格
- 专有名词：保持正确大小写(GitHub、TypeScript、Vue.js)
- 代码/数字/单位：使用半角字符，数字与单位间不加空格(16px、100ms)
